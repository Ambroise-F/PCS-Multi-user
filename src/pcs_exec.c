 ...
 */
void generate_random_key(mpz_t s, int nb_bits, int shift)
{
  //time_t t;
	mpz_t min;
	mpz_t max;
	...
	srand((unsigned) time(NULL)*shift); // shift to avoid getting the same seed on the same second on 2 different calls
