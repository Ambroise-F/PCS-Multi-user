...
void pcs_mu_init(point_t  P_init,
	...
	*/
	long long int pcs_mu_run_order(mpz_t x_res[__NB_USERS__], int nb_threads, unsigned long long int times[__NB_USERS__],unsigned long int pts_per_users[__NB_USERS__])
	{
		...
		// int userid1;
		// int* userid2;
		uint16_t userid1,userid2,userid_uptodate;
		...
		#pragma omp parallel \
		private( nb_pts, userid1, userid2, R, b, b2, x, r, xDist, xDist_str, trail_length, col) \
		shared( collision_count, X_res, trail_length_max, pts_per_users, userid_uptodate, tv1, tv2) \
		num_threads(nb_threads)
		{
				...
				double_and_add(&R, Q[0], b, E); // R = bQi
				trail_length = 0;

				while(userid_uptodate<__NB_USERS__)
				{
					if(is_distinguished_mu(R, trailling_bits, &xDist)) // xDist = R.x >> trailling_bits
					{
						...
						if(struct_add_mu(b2, &userid2, b, userid1, xDist, xDist_str)) // ajout de b dans la mémoire, b2 = b d'un autre point avec collision
						{
							...
							if(is_collision_mu(x, b, userid1, b2, userid2, trailling_bits)) // si b et b2 forment une vraie collision
							{
								...
								#pragma omp critical (userid_update)
								{
									userid_uptodate = userid_uptodate > userid1 ? userid_uptodate : userid1;
								}
							...
							else // dist pt no true coll (shouldn't happen?)
							{
								// update userid1
					...
					else // not a dist pt
					{
						...
						f(R, M[r], &R, E);
						trail_length++;
						if(trail_length > trail_length_max || userid_uptodate > userid1)
						{
							// update userid1
							if (userid_uptodate > userid1)
							{
